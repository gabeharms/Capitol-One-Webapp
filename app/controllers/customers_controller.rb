class CustomersController < ApplicationController
  ...
  def index
    if params[:search] == ''  #This is useful when user searches for something then deletes the search
      ...
      @customers = Customer.where('lower(first_name) LIKE ? OR lower(last_name) LIKE ? OR lower(email) LIKE ?', params[:search].downcase, params[:search].downcase, params[:search].downcase).paginate(page: params[:page])
